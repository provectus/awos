...
- **Subagent discovery (filesystem + plugins).** `commands/tasks.md` and `commands/tech.md` reference both `.claude/agents/` (project-local, parsed via frontmatter) _and_ the `Agent` tool's description block (plugin-provided agents, recognized by the `plugin-name:` prefix on `subagent_type`) — introspection only; no prompt reads agent files for anything beyond `name` + `description`.
...
| `pre-migration-v2/`   | Full pre-2.0 roadmap/hire footprint at migration version 2  | Disown-in-place end-to-end: command bodies, templates, wrappers, user roadmap, and the full `.mcp.json` (awos-recruitment entry included) all byte-identical after the update; migrations 003–005 skip; current commands still install |
