...
3.  **Discover Available Domain Experts:** Identify available specialist agents registered in the system. For each agent available:
    - Understand the agent's domain expertise (e.g., Python/FastAPI, React, Kotlin/Spring Boot, testing, etc.)
    - Build a mental registry of available agents and their capabilities
    - Note: If no specialist agents are available, all tasks will be assigned to the `general-purpose` agent
