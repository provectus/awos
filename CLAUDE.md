...
`/awos:product` is interview-only — it never reads code. `/awos:architecture` runs an unconditional codebase-gather pass on every run: in Creation Mode an existing stack becomes the architecture defaults, cited with evidence, and confirmed in normal review like any other draft; in Update Mode the same gather is diffed against the recorded architecture and each drift item is confirmed with the user (adopt / keep as recorded) — never applied silently. It also keeps reading `context/sources/sources.md` when a project has one configured — the `configure-external-sources` skill that writes it remains directly invocable (it is no longer called from an onboarding command) and today serves `/better:spec`'s research lane until `docs/2.0/roadmap-2.0.md` item 3.3 dissolves it in favor of runtime tool introspection.
...
The repo has **two independent version lines**. The npm installer version is managed by release-drafter via PR labels — never bump it manually. The **plugin version** is manual: when plugin behavior changes, bump it as one deliberate commit moving three files together — `.claude-plugin/marketplace.json`, `plugins/awos/.claude-plugin/plugin.json`, and the `EXPECTED_PLUGIN_VERSION` pin in `tests/lint-prompts.test.js` (the lint exists to force exactly this three-file discipline). The engine stamps the plugin version into `audit.json` provenance and the report footer, so a behavior change shipped without a bump mislabels its audits. Magnitude tracks what that stamp labels: deliberate scoring-regime changes (weights, categories, curves, new dimensions) move major or minor; scoring bug fixes and changes that leave audit results bit-identical (command-surface removals, orchestration text, docs) are a patch (decided 2026-09-15).
...
- `collectors/*.ts` — one per source (`git`, `ci`, `tracker`, `docs`, `incidents`); each queries its source once and emits a shared JSON artifact. Git is always available; absent sources SKIP with a reason.
...
## The Better Plugin (experimental)

`plugins/better/` is a second, experimental plugin — the `better` namespace hosts higher-rigor forks of core commands, of which `/better:spec` is the first: it forks the core `/awos:spec` flow and adds a parallel research fan-out before drafting (codebase via `Explore`, web via `WebSearch`/`WebFetch`, internal KB gated on `context/sources/sources.md` being `## Status: configured` with a non-manual `Category: documentation` source) and a post-write blind verification by the bundled `spec-verifier` agent (`tools: Read`, dispatched with only the spec file path). Question ordering is load-bearing: everything the research raises becomes a `[NEEDS CLARIFICATION: …]` marker carrying its research-backed assumption, and every question except the opening interview is asked from Step 8 onward, **after** both markdown files are written — under `claude -p` a dismissed question ends the turn, so a deliverable written after one never happens. The opening interview is a single batched ask, skipped entirely when `AWOS_UNATTENDED` is set. The deliverable contract is deliberately identical to `/awos:spec` — same template, same `context/spec/NNN-name/functional-spec.md` — plus two additive artifacts nothing downstream depends on: `research-notes.md` (raw findings the tech phase can draw on — nothing reads it automatically) and `functional-spec.html`, a human-facing review page rendered as the command's final step. The page is produced by `plugins/better/scripts/render-spec.mjs`, a no-dependency Node script that extracts contract text verbatim from the markdown (syntax-driven, template-agnostic) and layers on an in-session-authored view-model (at-a-glance, decisions, findings-with-impact, provenance badges, and an optional declarative diagram the renderer draws itself — view-model text is escaped, never interpolated as markup) passed as an ephemeral temp file — the view-model is never stored in the user's project. Rendering is unconditional but non-fatal; the page is a stamped point-in-time snapshot. After rendering, the command offers (gated on the `Artifact` tool being present) to publish the page as a Claude artifact via the renderer's `--artifact` fragment mode — strictly opt-in, with an unanswered question meaning no and unattended runs never publishing. Renderer contracts are tested in `tests/better-render.test.js`. Core commands are untouched by design; if the experiment proves out, the flow merges into core `spec.md`. Its version line is independent of the awos plugin's and moves as **three** files together — its marketplace entry, `plugins/better/.claude-plugin/plugin.json`, and the `EXPECTED_BETTER_PLUGIN_VERSION` pin in `tests/lint-prompts.test.js` (no generator constant).
