...
**Why a separate scored dimension.** This is a deliberate choice over two alternatives. A zero-weight lens (the tier annotations already exist, so the Prevention Matrix could be a pure report layer) would not move the score, and moving the score is the point — prevention only improves if it counts. Distributing the check pairs into their home dimensions (secrets-hygiene into application-security, etc.) would blur each dimension's subtotal by mixing two different questions — "how good is the state" and "how well is it protected" — and would scatter the cross-cutting matrix. The accepted cost of the separate dimension is double representation: the same concern appears once as state (e.g. AS-05, "are secrets committed") and once as prevention (PRV-01/PRV-11, "is there a gate / a written rule"). These are two distinct failures by design — a clean repo with no guard is one regeneration from a dirty one — and this duplication is why PRV checks are excluded from the org rollup's `org_gaps` (their org-level expression is `prevention_gaps`).
