...
`src/core/setup-orchestrator.js` runs six numbered steps: init → create directories → run migrations → copy files → configure MCP → register plugin marketplace. Each step lives in its own service module under `src/services/`. The orchestrator and `setup-config.js` are the two files to touch when changing setup behavior. The marketplace step (`marketplace-configurator.js`) also writes an `enabledPlugins` entry that enables the `awos-containment` plugin, so its PreToolUse containment hook is active after install — there is no separate settings-hook step.
...
## The awos-containment Plugin

`plugins/awos-containment/` is a second Claude Code plugin — a least-privilege containment lever. It ships one PreToolUse hook (`hooks/hooks.json` → `hooks/awos-containment-guard.js`, a dependency-free Node script) that blocks the harmful subset of tool actions during AWOS runs: out-of-tree writes, network egress to non-loopback hosts, secret-file reads, and in-tree writes that would disarm the guard or plant persistence. It fires even under `--dangerously-skip-permissions`, and for subagent tool calls, and carries an `AWOS_CONTAINMENT_OFF=1` operator escape hatch. The reason it is a hook rather than a `permissions.deny` rule is expressiveness, **not** that deny rules stop working under bypass — they don't stop working; Claude Code applies deny and explicit ask rules in every mode. A deny rule cannot carry allowlist exceptions, so "deny writes outside the project root, allow everything inside" and "deny egress except to loopback" are not expressible as deny rules. Note also that Claude Code's own protected-path list already covers `.claude/`, `.git/` and `.mcp.json` in every mode except `bypassPermissions`, so the guard's self-protection is duplicative outside that mode; `.github/workflows/` is not on that list. The guard is a containment layer, not a general injection defense — it does not stop a benign in-tree, in-domain write. The plugin is declared as a second entry in the root `.claude-plugin/marketplace.json`; the installer registers the marketplace unconditionally and, **subject to consent**, enables the plugin (writes `enabledPlugins`) so the hook is active. Enablement is a team-shared mutation — the `enabledPlugins` entry lives in the committed `.claude/settings.json`, so one install propagates to everyone on the repo — so it is consent-based: a TTY `[Y/n]` prompt (default-yes), a non-TTY default of enable (secure-by-default), `--containment`/`--no-containment` to short-circuit, and a **sticky** decision (an explicit `false` is never silently re-enabled on a later run). The runtime opt-out is `AWOS_CONTAINMENT_OFF=1` — not circular, because an in-run edit that would disarm containment is denied while the environment-variable hatch is not. `.claude/settings*.json` is **not** on the blanket protected list: a blanket deny there denied the rollback of a hook install while the install itself (performed inside an `npx` subprocess the command parse cannot see) went through — the guard denied the undo and permitted the do. Instead the settings files carry an invariant: any edit is allowed as long as containment is left enabled, registered, and hooks not switched off, evaluated over the merged `settings.json` + `settings.local.json` stack. A Bash redirect into those files carries no inspectable content and stays a blanket deny. Its version is bumped in **both** `.claude-plugin/marketplace.json` and `plugins/awos-containment/.claude-plugin/plugin.json`. The guard's unit test lives at `tests/installer/containment-guard.test.js`.
...
- `prevention.ts` — the prevention-coverage linkage pass: derives the per-cluster tier view (`enforced`/`instructed`/`absent`/`pending`) from the PRV check pairs, annotates covered source-dimension checks, and emits the `audit.prevention` block (audit.json only; per-dimension files stay raw). Purely operates on dimension objects; recomputed by both `audit-core` and `aggregate` (never preserved), which is how `pending` tiers finalize after `patch-judgment`. The PRV check records are self-describing (`cluster`/`covers_checks` stamped from standards.toml), so no standards access is needed post-audit.
