...
[Ticket sources: also fetch the **surrounding context** §1 recorded — the **epic/parent ticket's description, remote links, attachments, and linked conversations** (tracker remote links, attached design docs or screenshots, a referenced Confluence/Notion page, a linked chat thread) via the §7 transports — and read the reachable ones. This pre-seeds `/awos:spec` so its interview opens warm: the epic frames the "why", a design link or screenshot pins the "what". Treat any source whose §7 transport is an optional accelerator as best-effort: when it is unavailable, record which context went ungathered and carry on — surrounding context enriches the spec interview, it never gates the flow. Fold the reachable material into the normalized bundle, and **list anything linked but unreachable** — including anything gated behind a transport this run does not have — in it instead of silently skipping — the specs stage then knows context is missing rather than assuming the description is complete, and writes that list into the flow log's first entry. Step 4 passes the bundle to `/awos:spec`, so the richer material flows through without a second fetch. Omit this paragraph for local-file or prompt-text sources, and skip any surrounding-context source §1 records as "none".]
...
Run `/awos:verify` [per §8: in a subagent if it is non-interactive], returning the verdict and the list of gaps. On failed criteria `/awos:verify` appends a `Fix verification failures` slice to `context/spec/{SPEC_NAME}/tasks.md` (and reverts any stale Completed status) — close the loop mechanically rather than improvising fixes in this context: re-run `/awos:implement`, which picks up the unchecked fix tasks natively, then run `/awos:verify` again. Repeat until verification passes, capping at three rounds: after a third failed round the fix is not converging — stop, report the remaining gaps to the user [and per §9 notifications], and leave the unchecked fix tasks in `tasks.md` as the record of what is still open. The retry loop applies only to failed criteria that produced fix tasks. When the remaining gaps are criteria left `[?]` (manual confirmation declined or unanswered) with no fix slice appended, there is nothing for `/awos:implement` to execute — do not spend rounds on them: report the pending confirmations to the user [and per §9 notifications] and stop this stage. Log each round in this stage's flow-log entry (round count, which criteria failed), so a resumed session knows where the loop stood.
...
[Per §5 merge policy: a human merges — skip the flow-merge and proceed to the close stage, which reports the ready-to-merge state as the terminal evidence — or the flow merges via the chosen transport from §7: the platform's merge capability, or a plain `git merge` + push for a repo without a code host.]
