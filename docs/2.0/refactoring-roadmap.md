...
| 3     | ~~Remove `/awos:hire`~~ — **reverted 2026-09-17**                                                 | Struck: `/awos:hire` stays (see the Phase 3 note)                                                                |
...
| 6     | Consolidate the flow: 6A `implement` absorbs `tasks` + `verify`; 6B `better:spec` folds into core | 6A ungated (its "after Phase 3" gate went with the reverted phase); 6B when the better workstream calls it ready |
...
**Target main flow: `spec → tech → implement`** (foundation: `product → architecture → hire`). `tasks` and `verify` are not target-flow steps: `/awos:implement` absorbs both — it splits the scope itself, runs through it, and verifies the result, all in one. That consolidation is Phase 6A. And the spec step means `/better:spec` — **THE spec command**, WIP status notwithstanding (Daria, 2026-09-09); the roadmap plans around it, and it folds into core as Phase 6B. Phases 1–3 operate on the command set as it exists today.
...
- Plugin version bumped 2.4.5 → 2.4.6, moving three files (not four): `flow.md`'s generator-version constant no longer exists to anchor a fourth, so the lockstep discipline collapsed to `marketplace.json`, `plugins/awos/.claude-plugin/plugin.json`, and the `EXPECTED_PLUGIN_VERSION` test pin. (Revised 2026-09-15, Daria — originally cut as 3.0.0: the plugin version is audit provenance, stamped into `audit.json` and the report footer, and this wave leaves scoring bit-identical; removing surface the audit never scored is a patch, not a major. Same call for better: 0.1.0 → 0.1.1, not 0.2.0 — the roadmap-lane drop changes no deliverable contract.)
...
**As executed:** the spec-anchor decision — the roadmap lane is dropped from both spec commands (core `/awos:spec` and `/better:spec`); `topic` becomes the required anchor in its place. This resolves the "better workstream" dependency this section originally called out (see "The `better` workstream" below). The installer migration (003) replaces the project's local copy of the command body with the removal notice (creating it where only the wrapper remains) so `/awos:roadmap` answers that the feature left AWOS, and deletes the orphaned template; the wrapper and the user's roadmap document are left untouched (graceful shutdown, amended 2026-09-17 — class 1 below). The audit engine is untouched by this phase: `audit_patch.ts`'s comment referencing `/awos:roadmap` and the SDD detectors' roadmap-file checks are stale but ride until Phase 8 retires the audit stack.
...
## Phase 3 — ~~Remove `/awos:hire`~~ (reverted)

**Status: ~~executed 2026-09-10~~ reverted 2026-09-17** (Daria): removing `/awos:hire` completely was a mistake in sequencing — its fate is undecided, and it is kept as is, knowingly against the philosophy, until that decision is made. Restored: `commands/hire.md`, `claude/commands/hire.md`, `docs/commands/hire.md`, `templates/agent-template.md`, the installer's `awos-recruitment` MCP step (`setup-orchestrator.js` runs six steps again), the `/awos:hire` pointers in `tasks.md`, `tech.md`, `architecture.md`, README, and CLAUDE.md, and the hire lint contracts. Migrations 004 (hire wrapper repair) and 005 (orphaned-MCP cleanup) are dropped — they never shipped. What stays from this phase: `/awos:tasks` still surfaces a staffing gap as an open question and `/awos:implement` still falls back to `general-purpose` and reports it — both now point at `/awos:hire` as the way to fill the gap. The rest of this section is kept, struck, as the record of what was done and why.
...
**Status: executed 2026-09-10.** 14 PRs closed, each comment naming its principle and its kernel's home (verdicts in the table below, updated same day); #199 (one-line README de-hype) merged 2026-09-15; #189 closed outright, superseding the split ask; #194 untouched on its own track. The `major` release label had to be created (repo had only minor/patch). Exit criterion met: no open PR contradicts the philosophy or a planned phase.
...
| #199       | Merged (2026-09-15)            | One-line tagline de-hype — accurate, conflict-free with the cleaning rewrite; the surviving "production-ready" overclaim goes to the tone cleanup                                                                                                                                           |
...
- **Re-organize the survivors:** every remaining issue gets a roadmap-phase or registry-gap reference; a feature request gets the five-direction test before it earns a label; an issue that is really a diagnosis is recorded in this document's phase notes first — issues track scheduled work.
...
- Retire the now-empty plugin (flow left in Phase 1, sources in Phase 7 — if this gate opens first, the sources dissolution moves up): `plugins/awos/`, `.claude-plugin/marketplace.json`, the installer's marketplace-registration machinery (`src/services/marketplace-configurator.js`, orchestrator step 6 — the last of six; the `awos-recruitment` MCP step is step 5), and the plugin version line with its lint discipline. **Marketplace retirement additionally requires Phase 6B done** — the better plugin rides the same marketplace; until the spec folds into core, the marketplace stays. The marketplace registration in user settings follows the user-file policy.
...
| ----- | ---------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1     | `.awos/` internals                                                           | Migrations delete freely — the existing overwrite contract. ~~**Amended 2026-09-15 (Daria): internals of removed commands are disowned in place** — where present they are never deleted or rewritten (frozen 1.x copies keep working); migrations only repair broken wrapper imports with a removal notice.~~ **Amended 2026-09-17 (Daria): graceful shutdown.** A retired command's local body is replaced with the removal notice wherever the command or its wrapper is present, so calling it answers that the feature left AWOS instead of running a frozen copy; nothing of the user's is deleted — the wrapper and `context/product/roadmap.md` stay untouched; the orphaned template under `.awos/templates/` is removed, since nothing reads it once the command is a notice. The roadmap document is the team's own tool now: never created, updated, or marked by AWOS, but both spec commands keep offering a maintained roadmap's incomplete items as topic candidates **indefinitely — no sunset**: a compatibility read of a user-owned file, not a feature (a team that keeps its roadmap current for agentic SDLC is not forbidden from feeding it to spec). |
| 2     | Wrappers in `.claude/commands/awos/`                                         | ~~Deleted only when byte-identical to the shipped version; otherwise preserved with a notice~~ **Amended 2026-09-17:** a wrapper byte-identical to any shipped 1.x version is _rewritten_ to the 2.0 removal wrapper (its palette description says the command was removed; it still resolves to the notice, so calling it answers — deleting it would turn the graceful answer into the host's "unknown command"); a customized wrapper is preserved untouched and resolves to the same notice. Deleting either file is the user's move: the notice and the one-time update announcement name both                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
- _Mechanics (verified 2026-09-09):_ ~~migration preconditions are file-existence only, so class 2's byte-comparison cannot be a JSON migration — it lands in the installer's copy logic, which already compares content for the preservation policy. Migrations handle class 1 only.~~ Superseded 2026-09-17: `replace_content` gained an `if_sha256` guard (the hashes of every shipped 1.x wrapper), so class 2 is handled by migration 003 after all; the copy step never touches wrappers it no longer ships. The same migration carries the one-time user announcement (`notice`), so the installer holds no product knowledge of removed commands.
