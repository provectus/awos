...
3.  **Identify Subagents:** Based on the functional spec and architecture document, determine which technology stack(s) this feature will primarily involve. Discover all available agents in `.claude/agents/`, `~/.claude/agents/`, and `~/.claude/plugins/marketplaces/` directories and all subdirectories that match the feature's technology requirements.
