/**
 * Tests for pr_cycle_time metric.
 *
 * Contracts verified:
...
import { compute } from '../metrics/pr_cycle_time.ts';
...
test('adp_g5: < 24h cycle time → elite band', () => {
  ...
  assert.equal(result.metric, 'pr_cycle_time', 'metric id must match');
});
...
// ---------------------------------------------------------------------------
// Phase 3b: score/confidence contracts (identical formula to lead_time_for_change)
// ---------------------------------------------------------------------------
