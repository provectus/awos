...
It also updates `context/product/domain.md` and `context/product/glossary.md` — the rules around the feature and how it connects to the rest of the product, which the spec itself deliberately cannot carry.
...
2. **Gathers context**: Reads the product definition, the roadmap, and the description of how the product works (`context/product/domain.md`, `context/product/glossary.md`) so the interview starts from what you have already explained rather than from zero.
