...
1. **A git repo** (target folder has `.git`) that does **not** itself hold independent git repos in subdirectories → **single-repo mode** over that repo. A monorepo is this case — one repo, audited whole, because its subdirectories aren't independent repos. If the repo is itself a member of an orchestration root (a distinct parent work tree carrying agent tooling), the audit still runs in single-repo mode and the engine credits the root automatically — you pass no flag. (A repo that itself holds independent git repos in subdirectories is case 4, not this case.)
