...
- **Flow log:** `context/spec/{SPEC_NAME}/flow-log.md` — the flow's resumable memory; what each entry records and how it keeps the window small is defined in the generated command's Context Discipline section. Committed with the work and finalized at commit-push: the flow stops writing to it once the change request is opened or merged (new commits can't reach a merged/in-review change request), so it never becomes an uncommittable leftover; late-stage progress is reported and announced instead, and the remote stages resume from remote state
