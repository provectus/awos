...
- Retire the now-empty plugin (flow left in Phase 1, sources in Phase 7 — if this gate opens first, the sources dissolution moves up): `plugins/awos/`, `.claude-plugin/marketplace.json`, the installer's marketplace-registration machinery (`src/services/marketplace-configurator.js`, orchestrator step 5 — the last of five since Phase 3 removed the MCP step), and the plugin version line with its lint discipline. **Marketplace retirement additionally requires Phase 6B done** — the better plugin rides the same marketplace; until the spec folds into core, the marketplace stays. The marketplace registration in user settings follows the user-file policy.
