...
test('plugin.json version matches the awos marketplace entry and equals 2.4.2', () => {
  ...
  assert.equal(
    pluginManifest.version,
    '2.4.2',
    `plugins/awos/.claude-plugin/plugin.json version must be "2.4.2" (release version is managed by release-drafter, not bumped per change), got "${pluginManifest.version}"`
  );
});
