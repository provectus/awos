...
2. **The output dir is shared target state.** The skill writes into a datetime-stamped `context/audits/YYYY-MM-DD_HH-MM-SS/` under the target — there is no previous-audit or delta concept, so nothing there feeds a later run. For isolation the harness still blanks `context/audits/` before every run (stashing whatever was there into `_preexisting/`, never deleting) and archives the fresh output elsewhere; comparison is done from the archive.
...
Every run is the cold case: the audit has no previous-audit or delta concept, and its output dirs are datetime-stamped, so there is nothing to seed. Whatever is already in the target's `context/audits/` is **stashed into the run's `_preexisting/` first** (moved, never deleted) before the harness blanks it, so a run never sees another run's leftovers.
