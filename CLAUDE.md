...
- **Don't gratuitously name "Claude Code" inside prompts.** AWOS targets Claude Code only, so the host doesn't need to be re-introduced sentence by sentence. Paths like `.claude/agents/*.md`, conventions like the `plugin-name:` prefix on `subagent_type`, and tool names like `Agent` / `Read` / `Glob` already encode the host. Explicit "loaded by Claude Code" / "in Claude Code" attributions in the prompt body almost always just trim. The framework-level note that AWOS targets Claude Code belongs in _this_ file, not in every prompt.
