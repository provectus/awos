...
**As executed:** the spec-anchor decision — the roadmap lane is dropped from both spec commands (core `/awos:spec` and `/better:spec`); `topic` becomes the required anchor in its place. This resolves the "better workstream" dependency this section originally called out (see "The `better` workstream" below). The installer migration (003) deletes nothing — under the 2026-09-15 disown-in-place amendment (class 1 below) it only writes a removal notice where a preserved `.claude/commands/awos/roadmap.md` wrapper points at a body the project does not have; present 1.x copies of the command and template are left untouched. The audit engine is untouched by this phase: `audit_patch.ts`'s comment referencing `/awos:roadmap` and the SDD detectors' roadmap-file checks are stale but ride until Phase 8 retires the audit stack.
