...
- **Honest confidence, per-metric reliability.** Every number carries a confidence label (HIGH/MED/LOW) from which sources were reachable, and every metric carries a reliability tag — _minimal_ (true value ≥ shown), _maximal_ (true value ≤ shown), or _not-reliable_ (proxy) — with a "where it may deviate" note. In the HTML these appear as **hover hints** on every rendered number; the hint contains five parts: _definition · derivation · reliability (confidence) · source · method_. The same five-part hint is preserved in the Markdown output. The front page stays clean but nothing is overclaimed.
...
**Report generation is deterministic.** The audit engine writes a single `audit.json` file. The command `node dist/cli.js render audit.json --format html` (or `--format md`) converts it to the three-tab HTML or Markdown report — pure and deterministic: no clocks, no LLM calls. The JSON is the single source of truth; the HTML/Markdown are derived output. The report is never hand-written.
