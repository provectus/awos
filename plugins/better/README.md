...
1. **Research fan-out before drafting.** After a big-picture interview round, up to four agents run in parallel against the current understanding:
   - **Codebase** (always) — adjacent features, current behavior, in-repo docs, contradictions.
   - **Prior agreements** (always) — what has already been agreed about the behavior the topic touches: earlier specs in `context/spec/`, in-repo decision records, and the assertions of tests covering that behavior, which are the closest thing existing code has to recorded acceptance criteria. It looks for decisions about behavior, never for conventions or stack choices. Reports `NO PRIOR AGREEMENTS` when the repository records none.
