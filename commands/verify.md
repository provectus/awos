...
- **Output:** `context/product/domain.md` and `context/product/glossary.md` — updated for this feature, then reviewed as a whole
...
- **Look-and-feel means real rendering, with screenshots kept as evidence.** For any acceptance criterion describing something a user sees or does in a UI — a rendered page, a control, a state change, a redirect — in-process or component tests are **not** sufficient: they confirm logic, not look-and-feel. Start the app if it isn't running, drive the actual UI through whatever browser-automation tool the project ships (Playwright MCP/CLI, Cypress, the chrome MCP, etc.), and capture screenshots of the states the criteria describe. **Running the app is your job, not the user's — but do it safely.** Getting the app running so you can render a criterion is your responsibility, not something to punt with a `run` command handed to the user. Run it without disturbing what's already there: don't stop, restart, or reconfigure a service the user has running; don't mutate shared persistence (a live database, shared fixtures, on-disk state another instance depends on); and don't seize a contended port, device, or third-party resource. Prefer an already-running instance; otherwise start an isolated one (alternate port, throwaway/ephemeral data, a scratch target) and track it so you stop what you started and leave no orphan processes. The agent self-certifies only objective, checkable UI states — a control is present, a state toggles, a redirect fires. Whether the rendered result _looks_ the way the spec describes it (styling, layout, visual polish) is a judgment models make unreliably: capture the state to `docs/screenshots/`, then ask the user via `AskUserQuestion` to open that screenshot and confirm it matches the spec's described look-and-feel, rather than self-certifying the match. If a criterion genuinely can't be rendered without disturbing a shared resource, ask the user via `AskUserQuestion` rather than forcing it. Save them to `docs/screenshots/` — the same evidence folder the `testing-expert` agent writes E2E captures to — naming each file so it sorts by spec: `docs/screenshots/<spec-directory>-<short-state>.png` (e.g. `docs/screenshots/011-scheduled-tasks-amber-pill.png`). The browser tool creates the folder on first write; do not edit `.gitignore` — git-ignoring `docs/screenshots/` is a one-time project setup, not part of verification. Reference the saved paths in the report — they are what the human reviews.
...
### Step 6: Update and Review the Domain Description

The feature is verified, so the product now works differently than it did — and building it revealed things planning could not. This is also the one point in the cycle where reviewing the whole description is safe: nothing downstream is waiting on it.

1. Invoke `Skill(name="awos-writing-domain")`. If it is not found, say so and continue to the report.
2. **Update.** Bring the sections this feature touched up to date, including where the implementation diverged from the spec and why, and any constraint discovered only by building (an API limit, a library that could not do what was assumed).
3. **Review.** Follow the skill's review pass over the whole description: correct what has drifted, cut back what an artifact now states, fill a part that has no section, and hold the size budget. Surface a contradiction to the user rather than resolving it silently.
4. Report what changed.

Do not restate what was built — the code and the spec both carry that. Describe how the product now works, and why.
...
- **Domain description:** which sections Step 6 changed — the git diff on `context/product/` is how the user reviews it.
