...
- **Honour the `quick-verify` mode.** If the spec's `tasks.md` carries the `<!-- quick-verify: true -->` marker, verification is satisfied by the project's test suite: run it once, and if it exits zero, mark every acceptance criterion `[x]`. A green suite already demonstrates the implementation is correct, so walking each criterion against the spec repeats work the tests have done. Record "verified via test suite" as the evidence for each criterion.
...
**Quick mode runs first.** If `<!-- quick-verify: true -->` is present in `tasks.md`, run the project's test command. On a zero exit, mark all acceptance criteria `[x]`, record "verified via test suite", and go straight to Step 4 — skip the per-criterion walk below, including look-and-feel rendering and screenshots.
