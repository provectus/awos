...
| `pre-migration-v2/`   | Full pre-2.0 roadmap/hire footprint at migration version 2  | Graceful shutdown end-to-end for roadmap: migration 003 replaces the local command body with the removal notice while the template, wrapper, and the user roadmap stay byte-identical. Hire is current again, so its stale 1.x body and agent-template are overwritten by the copy step while the customized hire wrapper and the full `.mcp.json` (awos-recruitment entry already present) stay untouched; current commands still install |
