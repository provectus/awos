...
`plugins/better/` is a second, experimental plugin — the `better` namespace hosts higher-rigor forks of core commands, of which `/better:spec` is the first: it forks the core `/awos:spec` flow and adds a parallel research fan-out before drafting (codebase via `Explore`, web via `WebSearch`/`WebFetch`, internal KB gated on `context/sources/sources.md` being `## Status: configured`) and a post-write blind verification by the bundled `spec-verifier` agent (`tools: Read`, dispatched with only the spec file path). The deliverable contract is deliberately identical to `/awos:spec` — same template, same `context/spec/NNN-name/functional-spec.md` — plus two additive artifacts nothing downstream depends on: `research-notes.md` (raw findings for the tech phase) and `functional-spec.html`, a human-facing review page rendered as the command's final step. The page is produced by `plugins/better/scripts/render-spec.mjs`, a no-dependency Node script that extracts contract text verbatim from the markdown (syntax-driven, template-agnostic) and layers on an in-session-authored view-model (at-a-glance, decisions, findings-with-impact, provenance badges) passed as an ephemeral temp file — the view-model is never stored in the user's project. Rendering is unconditional but non-fatal; the page is a stamped point-in-time snapshot. After rendering, the command offers (gated on the `Artifact` tool being present) to publish the page as a Claude artifact via the renderer's `--artifact` fragment mode — strictly opt-in, with an unanswered question meaning no and unattended runs never publishing. Renderer contracts are tested in `tests/better-render.test.js`. Core commands are untouched by design; if the experiment proves out, the flow merges into core `spec.md`. Its version line is independent of the awos plugin's and moves as **three** files together — its marketplace entry, `plugins/better/.claude-plugin/plugin.json`, and the `EXPECTED_BETTER_PLUGIN_VERSION` pin in `tests/lint-prompts.test.js` (no generator constant).
