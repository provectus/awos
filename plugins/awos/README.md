...
> **If you have an existing `/implement-feature`, `/fix-bug`, or `context/product/delivery-flow.md`** — these were generated by the retired flow command. They're yours to keep, move, or delete; awos no longer generates or updates them. What the core installer does to a retired core command (the roadmap command, for instance) is described in [the upgrade guide](../../docs/2.0/upgrading-1.x.md).
