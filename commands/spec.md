...
- **Context Files 3:** `context/product/domain.md`, `context/product/glossary.md`, and `context/product/domain/*.md` if present — read before interviewing, updated after.
...
- **Read how the product works, before writing a single question.** `context/product/domain.md` describes the product part by part: what each does, the rules that govern it, how the parts connect, and what was deliberately decided. Read the sections your topic touches — and the ones it connects to, since a feature usually breaks against a neighbouring rule rather than its own. `context/product/glossary.md` holds the words this business uses; adopt them in the spec rather than inventing labels. If `context/product/domain/` exists, each file's frontmatter `description` says when it is worth reading. Everything you find here is a question you must not ask. **Finding facts is your job, not the user's; their time is for decisions only.** A question whose answer is already written down is the failure this step exists to prevent, and it is why users abandon the workflow.
...
- **Before asking questions:** Present a summary to the user: "Based on the roadmap and product definition, here's what I understand: [summarize known purpose, user capabilities, and context]. Let me clarify the remaining details." When the domain description supplied part of that understanding, say so — it shows the user their earlier explanations were kept, and lets them correct anything that has gone stale.
- Only ask questions whose answers are NOT already documented in the roadmap, the product definition, or the domain description.
...
### Step 7: Update the Domain Description

The spec says what this one feature does. The interview also revealed how the product works around it — rules, connections to other parts, choices made and rejected — and that belongs in the description, which outlives this spec.

1.  Invoke the skill: `Skill(name="awos-writing-domain")`. If it is not found, say so and stop this step — a missing skill costs the learnings, not the spec.
2.  Update the sections your topic touched: a rule the user stated, a connection to another part, a choice they made or rejected, a term they pinned down. Add a section only if this feature introduces a part the description has none for.
3.  Write the files the skill names.

Do not restate the feature's behaviour — the spec carries that, and a second copy competes with it. Record the rules and reasons around it, which the spec deliberately cannot.
