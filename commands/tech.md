...
3.  **Identify Domain Expert (if applicable):** Based on the functional spec and architecture document, determine which technology stack(s) this feature will primarily involve. Use Claude Code's native `/agents` command to discover available domain experts that match the feature's technology requirements.
