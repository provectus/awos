...
- **Subagent discovery (filesystem + plugins).** `commands/tasks.md`, `commands/tech.md`, and `commands/hire.md` reference both `.claude/agents/` (project-local, parsed via frontmatter) _and_ the `Agent` tool's description block (plugin-provided agents, recognized by the `plugin-name:` prefix on `subagent_type`). `hire.md` is the one prompt that reads agent files beyond `name` + `description` — it parses `skills:` frontmatter for its coverage table; the others introspect only.
...
| `pre-migration-v1/`   | `.claude/agents/python-expert.md` at the pre-v1 path        | Migrations 001 + 002 land cleanly; 003 is a no-op on this tree and the version file reaches the latest migration version                                                                                                                                                                                                                                                                                    |
| `pre-migration-v2/`   | Full pre-2.0 roadmap/hire footprint at migration version 2  | Disown-in-place end-to-end for roadmap: its command body, template, wrapper, and the user roadmap byte-identical after the update; migration 003 skips. Hire is current again, so its stale 1.x body and agent-template are overwritten by the copy step while the customized hire wrapper and the full `.mcp.json` (awos-recruitment entry already present) stay untouched; current commands still install |
