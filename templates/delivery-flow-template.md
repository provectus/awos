...
- **Supported:** manual — `/implement-feature <ticket-id-or-link>`
- **Wanted (setup notes only):** [polling via `/loop`/cron, webhook — what to set up, or "none". For unattended runs, the driver chains headless stage invocations (`claude -p "/implement-feature <ticket>"`; resume-detection makes each idempotent) — record the prerequisites the operator owns: the machine, `claude` on PATH, a permission mode that allows edits, and headless-capable transports]
