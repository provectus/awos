...
`plugins/profiler/` adds `/profiler:implement`, a skill that explains where an `/awos:implement` run spent its wall-clock. All numbers come from `plugins/profiler/scripts/profile-implement.mjs`, a no-dependency Node script over the Claude Code session transcripts (`~/.claude/projects/<slug>/<session>.jsonl` plus `<session>/subagents/agent-*.jsonl` and their `.meta.json`); the skill runs it, reads the Markdown report, and diagnoses. Three verbs: `sessions`, `report` (buckets, rates, long gaps classified active-versus-inactive by scanning every transcript on the machine, hangs, effort leaks, delegation chains, per-agent table, optional `--json`), and `watch` (polls and prints `DONE` / `STALL` / `EMPTY` / `IDLE` / `SUMMARY` / `RUN ENDED` lines for the `Monitor` tool, exiting when the orchestrator's completion line appears). Measurement rules worth knowing before editing: turns dedupe by `message.id` with output tokens as the max seen, because streaming writes several partial lines per turn; thinking text is redacted, so thinking share counts turns carrying a thinking block; gaps over 30 minutes are idle except a wait on a tool result, which is a hang unless nothing on the machine was active; test commands get a 20-minute hang threshold, everything else 10. The transcript format is undocumented, so `tests/profiler-script.test.js` runs the script against a synthetic session in the real on-disk shape and a drift fails loudly. The profiler observes only: it never kills a process or stops an agent. Its version line moves as three files, `plugins/profiler/.claude-plugin/plugin.json`, its marketplace entry, and the `EXPECTED_PROFILER_PLUGIN_VERSION` pin in `tests/lint-prompts.test.js`.
