...
1.  Construct a delegation prompt that includes:
    ...
    - A `<completion_evidence>` block: "A completion claim cites its evidence: run the verification commands and quote their actual output in your report. Never report behavior as working — no 'done', 'should work', or 'probably' — without fresh command output from this run proving it. If the task has you write a test, prove it with RED validation: temporarily revert the change the test covers, run the test and watch it fail, then restore the tree exactly and watch it pass — a test that also passes without that change captures nothing. Proving the tests you write is your job; the orchestrator's later spot-check is not a substitute for it." If the spec's `tasks.md` carries the `<!-- skip-tests: true -->` marker, drop the RED-validation sentences from the block — the team opted out of test suites, so no task writes one — but keep the evidence requirement: the subagent still proves behavior with a render, CLI, or MCP check instead of a test run.
