...
- `cli.ts` — the single dispatcher (`collect`/`detect`/`metric`/`standards`/`progress`/`render`/`rollup`/`audit-core`/`patch-judgment`/`report-context`/`patch-report`/`aggregate`/`enrich`). `render --format both --out-dir <dir>` writes `report.md` + `report.html` in one invocation.
