...
test(
  'Every computed metric must emit a human-readable expression so the report shows evidence (issue #12): adp_i1_work_mix',
  () => {
...
test(
  'Every computed metric must emit a human-readable expression so the report shows evidence (issue #12): adp_i2_throughput',
  () => {
    const tmp = makeTmpDir();
    const collectedDir = writeCollected(tmp, 'tracker', {
      tickets: [],
...
test(
  'Every computed metric must emit a human-readable expression so the report shows evidence (issue #12): adp_g2_contributors',
  () => {
    const tmp = makeTmpDir();
