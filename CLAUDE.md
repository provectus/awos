...
`/awos:product` is interview-only — it never reads code. `/awos:architecture` runs an unconditional codebase-gather pass on every run: in Creation Mode an existing stack becomes the architecture defaults, cited with evidence, and confirmed in normal review like any other draft; in Update Mode the same gather is diffed against the recorded architecture and each drift item is confirmed with the user (adopt / keep as recorded) — never applied silently. It also keeps reading `context/sources/sources.md` when a project has one configured — the `configure-external-sources` skill that writes it remains directly invocable (it is no longer called from an onboarding command) and today serves `/better:spec`'s research lane until Phase 7 of the refactoring roadmap dissolves it in favor of runtime tool introspection.
