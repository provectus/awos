/**
 * Tests for lead_time_for_change metric.
 *
 * Contracts verified:
...
import { compute } from '../metrics/lead_time_for_change.ts';
...
test('adp_g4: < 24h lead time → elite band', () => {
  ...
  assert.equal(result.metric, 'lead_time_for_change', 'metric id must match');
});
