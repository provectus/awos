...
/**
 * DORA band for the median lead time. Re-exported for org_rollup, which bands
 * the rolled-up median the same way.
 */
export const doraLeadTimeBand = doraDurationBand;
...
export function compute(
  ...
  const prLeadTimes = prDurationsHours(codeHost.prs, 'firstCommitMs');
  if (prLeadTimes.length > 0) {
    ...
    const expression = `median ${medianHours.toFixed(1)}h lead time over ${prLeadTimes.length} merged ${plural(prLeadTimes.length, 'PR')} from the code host (${band})`;
  ...
  const leadTimesHours = mergeRecordDurationsHours(
    raw.merge_records,
    windowStartMs
  );
  ...
  const expression = `median ${medianHours.toFixed(1)}h lead time over ${leadTimesHours.length} ${plural(leadTimesHours.length, 'merge')}${windowLabel} (${band})`;
  return makeMetricResult(
    'lead_time_for_change',
    medianHours,
    'banded',
