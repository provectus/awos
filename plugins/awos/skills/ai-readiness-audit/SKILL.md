...
The audit boundary is always the target folder or a GitHub org — never a manifest file. Resolve it from what the skill is pointed at (per `data-sources.md`). Check the cases in order — case 4's trigger is a strict superset of case 1's (both require `.git`), so it must be ruled out before case 1 can match:

1. **A git repo** (target folder has `.git`) that is not itself an orchestration root → **single-repo mode** over that repo. A monorepo is this case — one repo, audited whole. So is an ordinary project that happens to contain a nested git checkout — a vendored library, an examples app, a test fixture — with no agent tooling of its own governing it: containment alone doesn't make a target an orchestration root, only case 4's full trigger does. If the repo is itself a member of an orchestration root (a distinct parent work tree carrying agent tooling), the audit still runs in single-repo mode and the engine credits the root automatically — you pass no flag.
...
4. **An orchestration root** (target has `.git`, holds independent git repos in subdirectories, **and** carries the agent tooling that governs them — `.claude/`, `.mcp.json`, `.awos`, `context/`, or similar) → **org mode** over the root plus its member repos. Both conditions are required: containment without governing tooling is case 1, not this case — a repo that merely contains a vendored checkout or an examples app is one project, not a portfolio. This is the meta-monorepo layout: one repo carries `.claude/`, `.mcp.json` and the spec workspace, and the member repos it orchestrates sit in subdirectories that the root usually gitignores. Auditing the root alone would miss every member (walks honor `.gitignore`, and each member is its own work tree with its own `git log`), and auditing the members as peers would fail them for capability that lives at the root. So: the root is member #1, audited with `--no-orchestration-root`; every other member is audited with `--orchestration-root <rootPath>`.

The tooling condition above is presence-based, not a governance link — it checks that the target carries agent tooling somewhere, not that the tooling actually governs the specific nested repos found. A project with its own `.claude/` that also happens to vendor an unrelated checkout satisfies case 4's trigger as written; don't loosen the trigger to close that gap, a real governance link is a bigger change than a prose rule can carry. When the nested repos look incidental — `examples/`, `vendor/`, `third_party/`, test fixtures, sample apps — prefer case 1: members of a real orchestration root are the product's own services, referenced from the root's spec workspace and instructions, while incidental checkouts are neither. Phase 0b's scope confirmation is the safety net for a wrong call here, but only partially: interactively the resolved repo set is shown for correction, while a headless run proceeds on the auto-discovered set with no prompt — there, this judgment call is the only thing standing between an incidental checkout and an org-mode report for what is really one project.
...
- **Orchestration-root mode** (case 4): a flavor of org mode. Carry the resolved root path forward alongside the repo list — every per-repo dispatch in the Step 5 org branch needs it.
