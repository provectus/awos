...
**As executed:** the spec-anchor decision — the roadmap lane is dropped from both spec commands (core `/awos:spec` and `/better:spec`); `topic` becomes the required anchor in its place. This resolves the "better workstream" dependency this section originally called out (see "The `better` workstream" below). The installer migration (003) deletes nothing: it replaces the project's local copy of the command body with the removal notice (creating it where only the wrapper remains) so `/awos:roadmap` answers that the feature left AWOS; the template and the user's roadmap document are left untouched (graceful shutdown, amended 2026-09-17 — class 1 below). The audit engine is untouched by this phase: `audit_patch.ts`'s comment referencing `/awos:roadmap` and the SDD detectors' roadmap-file checks are stale but ride until Phase 8 retires the audit stack.
