...
1. **Picks the topic**: Uses your prompt as the feature topic, or asks you for one when the prompt is empty. In an unattended run with no topic, it stops with a one-line re-run instruction rather than guessing.
2. **Gathers context**: Reads the product definition to understand what's already documented about this feature.
...
5. **Creates the spec**: Runs a script to create the directory and saves the draft spec, then reviews it with you.

## Key behaviors

- **One feature per spec.** Each spec covers exactly one topic. Anything adjacent belongs in a separate spec.
...
- **"I can cover multiple features in one spec."** Each spec is scoped to one topic. This keeps specs focused and manageable.

## Example usage

```bash
# Good — no topic given; the command asks you for one:
> /awos:spec

# Good — name the feature to specify:
