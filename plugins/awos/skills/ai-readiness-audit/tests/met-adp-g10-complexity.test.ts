/**
 * Tests for adp_g10_complexity metric.
 *
 ...
 * - metric id is adp_g10_complexity
 *
...
import { compute } from '../metrics/adp_g10_complexity.ts';
...
test('adp_g10: metric id is adp_g10_complexity', async () => {
  if (!grammarsAvailable()) return;
  const tmp = makeTmpDir();
  ...
  assert.equal(result.metric, 'adp_g10_complexity');
});
