...
- **Interference** — two metrics are coupled so that no single project state can be extremal for both: the *same file* is a positive signal for A and a negative signal for B. You can max A or max B, never both.
...
### A4. Inverted range (scores *worse* as the repo improves)
...
These reward *small and simple*; a feature-complete, high-capability repo is necessarily larger and more complex, which pushes them down. They can only reach max on a trivial repo — the opposite end from where the rest of the audit wants to be. So "more AI-SDLC capability" and "top scale/complexity score" trade off.

**Diverge:** treat size/complexity as **descriptors** (informational), or band them so a *reasonable* size/complexity earns full credit and only *pathological* extremes are penalized — decoupling them from raw smallness.
