...
1. **A git repo** (target folder has `.git`) → **single-repo mode** over that repo. A monorepo is this case — one repo, audited whole. If the repo is itself a member of an orchestration root (a distinct parent work tree carrying agent tooling), the audit still runs in single-repo mode and the engine credits the root automatically — you pass no flag.
