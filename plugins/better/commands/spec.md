...
- A skipped or unanswered question is never a stop signal. Mark the unresolved detail with a `[NEEDS CLARIFICATION: …]` marker and continue through the remaining steps, including writing both output files. The one exception is the topic question inside the Step 3 batch: without a topic there is nothing to specify, so a dismissed batch that leaves the topic unanswered ends the run cleanly instead of becoming a marker.
...
- Put them in **one** batched `AskUserQuestion` call. When no topic was provided, the topic question opens that same batch — offer the most plausible candidates from the product definition as options, and, when a legacy `context/product/roadmap.md` exists (a user-maintained file AWOS neither creates nor updates), read it and include its incomplete items among the candidates, noting that AWOS no longer marks items complete, so an unchecked item may already be built; free text covers everything else. This is the only question the command asks before the files exist, so it is also the only one that can end an unattended turn before anything is written; one call is one such moment instead of several.
