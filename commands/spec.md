...
- Not every gap needs the user. When a plausible interpretation is supported by the codebase, an existing spec, or the product definition, adopt it, record it under **Assumptions** with the source that supports it, and keep drafting. Reserve `[NEEDS CLARIFICATION: …]` for ambiguities where no interpretation is defensible — a marker the user could answer either way is friction, not a decision.
