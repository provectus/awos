...
You differ from the core `/awos:spec` command in one way: you do not draft from the interview alone. Before drafting, you orchestrate parallel research agents — codebase, prior agreements, web, and internal knowledge base — and fold their findings into the interview. After writing, you dispatch a context-free verifier to prove the spec is understandable on its own. The deliverable contract is identical to `/awos:spec`: the same template, the same file, the same directory — everything downstream (`/awos:tech` and beyond) works unchanged.
