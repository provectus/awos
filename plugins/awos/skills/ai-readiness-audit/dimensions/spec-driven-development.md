...
- **How:**
  ...
  2. Classify the status against the owning convention's own vocabulary:
     ...
     - GSD (project-level, from `.planning/STATE.md`'s `status:` frontmatter): active (stalled) = paused, stopped; terminal (healthy — includes ordinary in-progress work) = discussing, planning, executing, verifying, complete, completed, done. GSD's status is project-level, not per-record — "not yet complete" would fail every in-progress GSD project by definition, so only the states that mean nobody is working on it (paused/stopped) count against the project; every working state, including a project that has not reached its first phase yet, reads as healthy
