...
You differ from the core `/awos:spec` command in one way: you do not draft from the interview alone. Before drafting, you orchestrate parallel research agents — codebase, web, and internal knowledge base — and fold their findings into the interview. The codebase agent carries a second duty: it reports what the project already agreed about the behavior the topic touches, and judges each agreement against the topic. After writing, you dispatch a context-free verifier to prove the spec is understandable on its own. The deliverable contract is identical to `/awos:spec`: the same template, the same file, the same directory — everything downstream (`/awos:tech` and beyond) works unchanged.
...
- **Both markdown deliverables are written before any question that could end the run unanswered.** Steps 1–7 ask nothing that can end the turn: the only interview is Round One (Step 3), asked whenever the run is interactive and skipped outright when it is not, and everything research raises becomes a `[NEEDS CLARIFICATION: …]` marker carrying its assumption. Every question from Step 8 onward refines files that already exist. Under `claude -p` a dismissed question ends the turn, so a deliverable written after one never happens — this ordering, not the marker rule above, is what makes an unattended run produce files. The HTML render (Step 10) runs last so the page reflects the final content.
...
1. **Codebase lane (always dispatched).** `Agent(subagent_type="Explore")` with the brief and a charge in two halves, reported as two separate lists.

   **Half one — what the code does today.** Find the features and behavior adjacent to this topic; how the product currently behaves in the areas the topic touches; any in-repo documentation about it; and anything that contradicts or complicates the brief. Label each item `[code]`.

   **Half two — what the project already agreed.** Find what has already been decided about the behavior this topic touches, from three in-repo sources: earlier specs under `context/spec/*/functional-spec.md`, whose requirements and acceptance criteria are literal prior agreements; in-repo decision records (architecture decision records, design docs, changelogs, and commit messages that explain why a behavior is the way it is rather than describe a change); and the assertions in tests covering the touched behavior, which are the closest thing existing code has to recorded acceptance criteria. Report what was agreed, the file that records it, and — where the source states one — the reason behind it, then close it with a verdict against the topic in the brief: `intact` when the topic leaves the agreement standing, `changed` when the topic alters it, `unclear` when you cannot tell from the brief. Judge the agreement against the topic only; do not judge whether the change is a good idea. Label each item `[prior]`, carrying its verdict. This half looks for decisions about behavior, not for conventions, stack choices, or code structure: how things are done here is the host's context to carry, and a finding of that kind belongs in half one or nowhere. If the repository records no agreements, return `NO PRIOR AGREEMENTS` for this half rather than padding the list.
...
3. **Internal-KB lane (dispatched when configured).** Dispatch iff `context/sources/sources.md` exists with `## Status: configured` and lists at least one source with `Category: documentation` whose `Access:` is not `manual`. When every documentation source is `Access: manual` there is nothing for an agent to retrieve — skip the dispatch and handle those sources through the Step 9 invitation. Copy each such source's `Platform:`, `Access:`, `Tool:`, and `Scope:` lines into the prompt of one `Agent(subagent_type="general-purpose")` call, charged with: retrieve what the organization's knowledge base says about this topic — prior decisions, related designs, naming, constraints. Return a terse findings list, each item labeled `[kb]` with the source. If a recorded transport is unreachable at runtime, return `SKIPPED: <source> unreachable` for that source rather than failing. Sources with `Access: manual` are excluded from the agent — record them for Step 9, which invites the user to paste relevant content after the files are written; note in `research-notes.md` that they were not consulted. If every documentation source is manual, the lane's outcome is `MANUAL SOURCES ONLY`, not `SKIPPED` (no agent ran to report it) and not `NOT CONFIGURED` (the user configured sources correctly). When `sources.md` is absent or not `configured`, note the lane as `NOT CONFIGURED` in the final summary and move on — never treat it as an error.
...
1. Merge the findings, keeping their origin labels (`[code]` / `[prior]` / `[web]` / `[kb]`). Set aside the raw technical form of every finding for `research-notes.md` (Step 7).
2. Sort the findings into four piles:
   ...
   - **Superseded agreements** — every `[prior]` finding the lane returned as `changed` or `unclear`, with no re-judging: the agent that read the source made that call against the same topic, so a finding does not leave this pile because the supersession looks obvious or looks minor. Do not resolve it on evidence the way a conflict is resolved: both sides are right, the agreement held when it was made and the topic changes it. Draft the new behavior and mark it: `[NEEDS CLARIFICATION: <the prior agreement>, agreed in <source>; this topic changes it to <the new behavior>. Confirm the supersession, or keep the agreed behavior.]` Mark it even when the topic states the change outright: the topic is the request, not the confirmation. Asking for a behavior is not the same as knowing it reverses something the project already agreed, and the person who wrote the topic is usually the one who does not know. Silently rewriting an agreement is the failure this pile exists to prevent — someone chose that behavior, and someone may depend on it. Naming the superseded agreements in the overview as well is useful, but it does not replace the marker: a line the reader can skim is not a question the reader has answered. A prior agreement the topic leaves intact is not a marker: restate it in the draft only where a requirement depends on it, and let the rest stand.
...
Then self-review the full draft end to end. First count: every `[prior]` finding the lane returned as `changed` or `unclear` must have a marker in the draft, and the counts must match — a large research run is exactly where one of these goes missing, and a supersession that reaches the reader as a note instead of a question has lost the only thing that made it an agreement. Then: replace any developer-facing language that slipped in; make vague or unmeasurable wording concrete in user-perceivable terms or convert it to a `[NEEDS CLARIFICATION: …]` marker; confirm every requirement carries at least one acceptance criterion. Then re-read the acceptance criteria one bullet at a time — not the set as a whole — and rewrite any bullet that does not carry both `when` and `then` in a single sentence. Checking the set as a whole is how a single non-compliant bullet survives.
...
4. Write `research-notes.md` into the same directory, from the raw findings retained in Step 5:
   ...
   A lane that did not produce findings records `SKIPPED: <reason>` (web/kb), `NOT CONFIGURED` (kb), or `NO PRIOR AGREEMENTS` (prior) under its heading. Technical language — file paths, component names, API shapes — is allowed and expected here.
...
2. Fold its findings back into the spec:
   ...
   - Judgment calls — a genuine ambiguity only the user can resolve — ask via `AskUserQuestion`. Both markdown files are already saved, so they survive whatever the answer is. Do not assume the run continues past an unanswered one: if the turn ends here, no later step runs, so write any marker you intend to leave into the file before you ask, not after.
...
1. Author the **view-model** — the re-shaped, human-friendly representation of what this session learned. Write it as JSON to a temporary file outside the project, following the schema documented at the top of the renderer script. Resolve the temp paths once, with a fallback for an unset `TMPDIR`, and reuse those exact paths for every write, render, and cleanup below:
   ...
   - `decisions` — every judgment call made in this spec (the things a reviewer might challenge): the decision, the choice, why, and its sources (`interview` / `code` / `prior` / `web` / `kb`).
...
5. Report: all three saved paths, the artifact URL if one was published, any research lane that was `SKIPPED`, `NOT CONFIGURED`, or `MANUAL SOURCES ONLY` (configured documentation sources exist but all are `Access: manual`, so no agent ran — the user was invited to paste from them in Step 9), any verifier findings left unresolved, and the next command: `/awos:tech`. Note that the HTML is a point-in-time snapshot stamped with its generation date — it does not update when the spec is later edited or verified.
