...
Core `/awos:spec` derives a spec from a one-line prompt or roadmap item plus an interview — details that live in the codebase, in what the project already agreed, in domain conventions, or in the organization's knowledge base never make it in. This plugin closes that gap.
...
1. **Research fan-out before drafting.** After a big-picture interview round, up to three agents run in parallel against the current understanding:
   - **Codebase** (always) — two duties, reported apart. First, adjacent features, current behavior, in-repo docs, contradictions. Second, **what the project already agreed** about the behavior the topic touches: earlier specs in `context/spec/`, in-repo decision records, and the assertions of tests covering that behavior, which are the closest thing existing code has to recorded acceptance criteria. Each agreement closes with a verdict against the topic — `intact`, `changed`, or `unclear`. The second duty looks for decisions about behavior, never for conventions or stack choices, and reports `NO PRIOR AGREEMENTS` when the repository records none.
   ...
   - **Internal KB** (when configured) — runs only when `context/sources/sources.md` exists with `## Status: configured` and lists a documentation source whose `Access:` is not `manual` — a manual-only source has nothing for an agent to fetch, so the lane reports `MANUAL SOURCES ONLY` and you are invited to paste from it during the final review instead. That file is written by the `configure-external-sources` skill, which ships with the **awos plugin** (not this one and not AWOS core) — install it with `/plugin install awos@awos-marketplace`, then run its `configure-external-sources` skill (or let `/awos:product` trigger it on brownfield projects). Reports `SKIPPED` when a recorded transport is unreachable.
2. **Synthesis into the draft.** Uncontroversial findings (edge cases, boundary behavior, error paths) flow straight into requirements and acceptance criteria. Findings that raise a real question — a conflict with what you said, or a choice you have not made — are drafted on the research-backed answer and marked, then put to you after the spec is saved. A prior agreement the topic _changes_ is handled apart from those: the new behavior is drafted, and the marker asks you to confirm the supersession or keep what was agreed, so an existing agreement is never rewritten silently. Technical findings are translated to user-facing language before entering the spec.

   This ordering is deliberate: the command asks nothing that can end the run before both markdown files exist. In a headless `claude -p` run, a question nobody answers ends the turn, and anything written after it never happens — so the spec is written first and refined second. An unattended run still produces a complete, coherent spec with every assumption named in a `[NEEDS CLARIFICATION: …]` marker. Export `AWOS_UNATTENDED=1` for such runs and the opening interview is skipped too.
...
| `research-notes.md`    | Additive side artifact: raw technical findings (`## Codebase findings`, `## Prior agreements`, `## Web research findings`, `## Internal KB findings`, `## Unresolved conflicts`). Nothing depends on it; the technical spec phase can draw on it. |
