...
> **Adopting AWOS on an existing codebase?** Run the audit before the foundation commands to get a baseline score and a punch list, then re-run it after adoption and compare the two reports. Its scoring is self-contained, so it works the same on an untouched repo.
...
> **If you have an existing `/implement-feature`, `/fix-bug`, or `context/product/delivery-flow.md`** — these were generated by the retired flow command. They're yours to keep, move, or delete; awos no longer generates or updates them. Likewise, if your project predates this version, the update preserves your roadmap and hire commands in place — wrappers, command bodies, and templates stay frozen at 1.x and keep working; awos no longer ships or updates them, and you migrate to the replacement paths on your own schedule.
...
├── agents/
│   └── repo-auditor.md          # org-mode per-repo audit subagent
├── skills/
│   ├── ai-readiness-audit/
...
│   └── configure-external-sources/  # external-source setup, directly invocable
└── README.md
```
