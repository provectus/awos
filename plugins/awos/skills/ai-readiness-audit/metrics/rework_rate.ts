/**
 * rework_rate — Deployment rework rate proxy (DORA 2024, git keyword proxy).
 *
 ...
 *   - change_failure_rate (g7): measures deployments needing immediate intervention
 ...
 *   - code_churn (g6): measures line-level code turnover (insertions deleted within the
...
export function compute(
  ...
  if ('error' in read) {
    return skipMetric('rework_rate', 'banded', 'minimal', 'git', read.error);
  }
  ...
  if (!ws || typeof ws.merges !== 'number' || ws.merges === 0) {
    return skipMetric('rework_rate', 'banded', 'minimal', 'git');
  }
