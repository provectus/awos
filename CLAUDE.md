...
| `skills/`          | `.awos/skills/`          | Framework internals. Overwritten on every update.                       |
| `claude/skills/`   | `.claude/skills/`        | Thin wrappers. User-editable customization layer — preserved on update. |
...
Skills work the same way, for reference material that several commands share — rules and formats that would otherwise be restated in every command that needs them and drift apart as they are edited. The body lives in `skills/<name>/SKILL.md`; the wrapper in `claude/skills/<name>/SKILL.md` carries the frontmatter plus a single `@.awos/skills/<name>/SKILL.md` import. Adding a skill means adding both, and the wrapper's `description` must match the body's — it is the wrapper the host reads, so a drifted copy changes when the skill fires. A command reaches one with `Skill(name="<skill-name>")` and carries only the trigger, not the rules. Shipped skill names carry an `awos-` prefix (`awos-writing-learnings`), enforced by lint — wrappers land in the user's own `.claude/skills/` alongside skills they wrote, and a name collision silently replaces one with the other.

Skill wrappers land un-namespaced at `.claude/skills/<name>/` because that is where the host discovers project skills — there is no `awos/` subdirectory as there is for commands, so a shipped skill's name must be distinctive enough not to collide with the user's own. One consequence worth knowing: the host only watches skill directories that existed when the session started, so the update that first creates `.claude/skills/` needs one editor restart before the skill resolves.
