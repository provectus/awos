...
2.  **Determine Topic:**
    ...
    - If the `<user_prompt>` tag is **empty** and the run is interactive, ask the user for the topic via `AskUserQuestion` — offer the most plausible candidates from `context/product/product-definition.md` as options, and, when a legacy `context/product/roadmap.md` exists (a user-owned file — 2.0 neither creates, updates, nor requires it; this is a compatibility read, not a feature), read it and include its incomplete items among the candidates, noting that they may be stale — 2.0 no longer marks items complete, so an unchecked item may already be built — at most four listed options in total (the tool's cap), keeping the most plausible; free text covers everything else. If the question is dismissed, the run ends with it — the topic exception in `# INTERACTION`, not a marker.
