...
In **org mode** (the target is a non-git folder of repos, a GitHub org, or an orchestration root — a git repo that itself carries the agent tooling governing independent member repos nested in its subdirectories) the orchestrator dispatches one `awos:repo-auditor` subagent per repo — each runs the same single-repo flow into `per-repo/<repo>/` — then aggregates with `rollup` into `org-portfolio.json` and an org-level report.
