...
4.  **Review requirements.** Which gates run, and in what order. The gates are combinable — when several remain open, ask them as one `multiSelect` question, not a series of yes/no picks. Settle these sub-decisions:
    ...
    - **Approval gates** — where the flow pauses for document review: gates after `/awos:spec` and after `/awos:tech` (most reliable: a good functional spec is the hardest artifact to get right on the first pass, especially when the ticket leaves things unconsidered, and an error caught at the spec gate never propagates — fixing spec and tech together later is much harder), one gate after spec and tech are both generated (faster — fewer waits for the user to return; the risk is reworking two documents together when the spec misread the ticket), or none (unattended runs, pre-written specs). `tasks.md` gets no gate by default — it rarely needs human review and `/awos:implement` starts right after `/awos:tasks`; state this default in the Step 4 confirmation summary so the user can object.
