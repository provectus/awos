...
> **If you have an existing `/implement-feature`, `/fix-bug`, or `context/product/delivery-flow.md`** — these were generated by the retired flow command. They're yours to keep, move, or delete; awos no longer generates or updates them. Likewise, if your project predates this version, the update replaces the local body of your roadmap command (`.awos/commands/roadmap.md`) with a removal notice — the command now tells you the feature left awos — and removes the orphaned roadmap template, while your wrapper and your `context/product/roadmap.md` stay untouched; a roadmap you keep maintaining is still offered as spec topic candidates.
