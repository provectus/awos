...
| #189       | Judge: correctness vs. surface | A scoring fix to output users keep receiving during the beta window — merge-worthy if it fixes wrong output; close if it adds surface    |
...
| #199       | Triage individually            | Small README fix; merge or close on content                                                                                              |
| #144       | Triage individually            | Delegate-by-path: passed the fidelity test in review, but not pre-approved here                                                          |
| #171       | Triage individually            | `/awos:quick`: principle 11's escape hatch is _the host's own planning_, not an awos command — possibly the "second path" regression     |
...
- **File what this roadmap owes:** the better-workstream coordination issues (the Phase 2a `roadmap.md` dependency, the Phase 7.6 KB-lane change).
